...
class AdsSearch(LoginRequiredMixin, PermissionRequiredMixin, ListView):
    ...
    def get_queryset(self) -> QuerySet:
        """
        Переопределяем метод для поиска рекламной кампании по вхождению
        подстроки в названии кампании.
        """
        query = self.request.GET.get("query")
        return (Advertise.objects.only("id", "name")
...
class StatisticView(LoginRequiredMixin, PermissionRequiredMixin, ListView):
    ...
    def get_queryset(self) -> QuerySet:
        """
        Переопределяем метод, для получения нужных нам данных, и
        сортируем по самым доходным рекламным кампаниям.
        """
        statistics: QuerySet | Any = cache.get_or_set(
            "statistics",
            Advertise.objects.annotate(
                customers_count=Count(
                    "leads__leads__lead_id",
                    distinct=True
                )
            )
...
class DeleteAds(LoginRequiredMixin, PermissionRequiredMixin, DeleteView):  # type: ignore
    """
    Класс для удаления рекламной компании. Комментарий type: ignore нужен
    для mypy, который прокидывает ошибку
