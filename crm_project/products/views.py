...
class ListProducts(LoginRequiredMixin, PermissionRequiredMixin, ListView):
    """
    Класс для отображения страницы со списком предоставляемых услуг.
    Страницу может просматривать пользователь у которого есть разрешение
    'view_products'.
    """
    ...
    def get_queryset(self) -> QuerySet:
        """
        Переопределяем метод, чтобы сначала показывать услуги которые наиболее
        пользуются спросом, (чем больше заключено контрактов с этой услугой).
        """
        return (
...
class ProductsSearch(LoginRequiredMixin, PermissionRequiredMixin, ListView):
    """
    Класс для отображения страницы со списком найденных по имени
    предоставляемых услуг. Страницу может просматривать пользователь у
    которого есть разрешение 'view_products'.
    """
    permission_required: str = "products.view_products"
    template_name: str = "products/products-list.html"
    model: Any = Products
    ...
    def get_context_data(self, *, object_list=None, **kwargs) -> dict:
        """
        Добавляет идентификатор для отображения введенного пользователем
        слова для поиска.
        """
...
class DeleteProducts(LoginRequiredMixin, PermissionRequiredMixin, DeleteView):  # type: ignore
    ...
    Комментарий type: ignore нужен для mypy, который почему-то выдает в данном
    месте ошибку 'Definition of "object" in base class "DeletionMixin" is
    ...
    def post(self, request, *args, **kwargs) -> HttpResponse:
        """
        Переопределил метод, для того чтобы при удалении услуги проверять нет
        ли действующих контрактов по данной услуге, если есть то удалить
        запрещаем.
        """
...
class CreateProducts(LoginRequiredMixin, PermissionRequiredMixin, CreateView):
    """
    Класс для создания услуги, доступен пользователям с разрешением
    'add_products'.
    """
