...
class CreateContract(LoginRequiredMixin, PermissionRequiredMixin, CreateView):
    ...
    def form_valid(self, form) -> HttpResponse:
        ...
        if form.is_valid():
            ...
            form.add_error(
                "end_date",
                "Дата окончания меньше чем дата начала контракта."
            )
        return render(
            self.request,
            "contracts/contracts-create.html", context={"form": form}
        )


class ContractsSearch(LoginRequiredMixin, PermissionRequiredMixin, ListView):
    """
    Класс для поиска контракта по названию. Поиск осуществляется по
    вхождению подстроки в строке без учета регистра.
    """
    ...
    def get_queryset(self) -> QuerySet:
        ...
        return (
            Contracts.objects.filter(Q(name__iregex=query))
            .order_by("-start_date")
        )
